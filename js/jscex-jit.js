(function () {
  ...
  var init = function (root) {
    ...
    JscexTreeGenerator.prototype = {
          ...
          if (
            ...
            if (newStmt.type == "raw") {
              ...
              if (isLast) {
                ...
                this._visitStatements(
                  statements,
                  combineStmt.second.stmts,
                  index + 1
                );
              }
            }
      ...
      _visitors: {
        ...
        "for-in": function (ast) {
          ...
          var keysAst = root.parse(
            "var " + keysVar + " = Jscex._forInKeys(obj);"
          )[1][0];
    ...
    CodeGenerator.prototype = {
      ...
      generate: function (params, jscexAst) {
        ...
        this._writeIndents()._writeLine(
          "var " +
            this._builderVar +
            " = Jscex.builders[" +
            stringify(this._builderName) +
            "];"
        );

        this._writeIndents()._writeLine(
          "return " + this._builderVar + ".Start(this,"
        );
        this._indentLevel++;
      ...
      _visitRawBody: function (body) {
        if (body[0] == "block") {
          ...
          this._writeIndents()._visitRaw(body);
          this._indentLevel--;
        }
      ...
      _jscexVisitors: {
        ...
        bind: function (ast) {
          var info = ast.info;
          this._write(this._builderVar + ".Bind(")
            ._visitRaw(info.expression)
            ._writeLine(", function (" + info.argName + ") {");
          this._indentLevel++;

          if (info.assignee == "return") {
            this._writeIndents()._writeLine(
              "return " + this._builderVar + ".Return(" + info.argName + ");"
            );
          } else {
            if (info.assignee) {
              this._writeIndents()
                ._visitRaw(info.assignee)
                ._writeLine(" = " + info.argName + ";");
            }
        ...
        if: function (ast) {
          ...
          if (ast.elseStmts) {
            ...
            this._writeIndents()._writeLine(
              "return " + this._builderVar + ".Normal();"
            );
          }
        ...
        try: function (ast) {
          ...
          if (ast.finallyStmt) {
            this._writeIndents()._visitJscex(ast.finallyStmt)._writeLine();
          } else {
            this._writeIndents()._writeLine("null");
          }
          this._indentLevel--;
        ...
        throw: function (ast) {
          this._write(this._builderVar + ".Throw(")
      ...
      _rawVisitors: {
        ...
        return: function (ast) {
          if (this._pos.inFunction) {
            ...
            this._write("return ")
              ._visitJscex({ type: "return", stmt: ast })
              ._write(";");
          }
        },
        ...
        break: function (ast) {
          if (this._pos.inLoop || this._pos.inSwitch) {
            ...
            this._write("return ")
              ._visitJscex({ type: "break", stmt: ast })
              ._write(";");
          }
        },
        ...
        continue: function (ast) {
          if (this._pos.inLoop) {
            ...
            this._write("return ")
              ._visitJscex({ type: "continue", stmt: ast })
              ._write(";");
          }
        },
        ...
        throw: function (ast) {
          ...
          if (pos.inTry || pos.inFunction) {
            ...
            this._write("return ")
              ._visitJscex({ type: "throw", stmt: ast })
              ._write(";");
          }
        },
        ...
        try: function (ast) {
          ...
          if (finallyStatements) {
            this._writeIndents()._writeLine("} finally {");
            this._indentLevel++;
    ...
    function _isJscexPattern(ast) {
      ...
      if (
        !compileMethod ||
        compileMethod[0] != "dot" ||
        compileMethod[2] != "compile"
      )
        return false;

      var jscexName = compileMethod[1];
      if (!jscexName || jscexName[0] != "name" || jscexName[1] != "Jscex")
        return false;
    ...
    function compile(builderName, func) {
      ...
      var evalCode =
        "eval(Jscex.compile(" + stringify(builderName) + ", " + funcCode + "))";
  ...
  if (isCommonJS) {
    ...
    define("jscex-jit", ["js/jscex-parser"], function (parser) {
      return {
        init: function (root) {
          if (!root.modules["parser"]) {
            parser.init(root);
    ...
    if (!Jscex.modules["parser"]) {
      throw new Error(
        'Missing essential components, please initialize "parser" module first.'
      );
    }
